...
inline SafetyResult isSafeCommand(const QString &cmd) {
  ...
  // Literal substring patterns — matched via QString::contains()
  ...
  // Regex patterns — for pipe-to-shell remote code execution
  static const QList<QPair<QRegularExpression, QString>> REGEX_PATTERNS = {
      {QRegularExpression("wget\\s+.*\\|\\s*bash"), "Remote code execution via wget|bash"},
      {QRegularExpression("curl\\s+.*\\|\\s*bash"), "Remote code execution via curl|bash"},
      {QRegularExpression("wget\\s+.*\\|\\s*sh"),   "Remote code execution via wget|sh"},
      {QRegularExpression("curl\\s+.*\\|\\s*sh"),   "Remote code execution via curl|sh"},
  };

  for (const auto &[regex, reason] : REGEX_PATTERNS)
    if (regex.match(c).hasMatch())
      return {false, reason};
